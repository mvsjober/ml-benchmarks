#!/bin/bash
#SBATCH --nodes=2 --ntasks=8 --cpus-per-task=10 -p gputest --gres=gpu:v100:4,nvme:200 -t 15 --mem=32G
...
srun --ntasks=$SLURM_NNODES --ntasks-per-node=1 tar xf $IMAGENET_TAR -C $LOCAL_SCRATCH
